...
void BNpc::setDir(Dir value)
{
    ...
    if(this->state==HState::walk) this->walk();
...
Vec2 BNpc::getEndPos()
{
    if(dir==Dir::left)   return Vec2(-150,0);
    if(dir==Dir::right)  return Vec2(150,0);
    if(dir==Dir::top) return Vec2(0,150);
    if(dir==Dir::bottom) return Vec2(0,-150);
    return Vec2(0,0);
}
